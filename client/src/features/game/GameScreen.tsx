...
export function GameScreen() {
  ...
  return (
    <div className="max-w-[480px] mx-auto px-5 py-5" style={{ backgroundColor: '#0f0f1a', minHeight: '100vh' }}>
      ...
      {currentTemplate && match.phase === 'action' && (
        ...
        >
          <div className="text-xs uppercase tracking-widest" style={{ color: '#6b6b80' }}>Your Role</div>
          <div className="text-xl font-bold my-1" style={{ color: '#e94560' }}>{currentTemplate.name}</div>
          <div 
            className="text-xs uppercase" 
            style={{ 
              color: currentTemplate.alignment === 'hero' ? '#4ade80' : 
                     currentTemplate.alignment === 'villain' ? '#e94560' : '#fbbf24'
            }}
          >
          ...
          return (
            <div
              ...
              style={{ 
                ...
                border: isDead ? '2px solid #6b6b80' : isSelected ? '2px solid #e94560' : '2px solid #2a2a4a',
      ...
      {match.phase === 'action' && currentTemplate && (
        <div className="rounded-2xl p-5 mb-5" style={{ backgroundColor: '#16213e', border: '2px solid #2a2a4a' }}>
          <div className="text-sm font-semibold mb-3" style={{ color: '#a0a0b8' }}>Your Abilities</div>
          <div className="flex flex-wrap gap-2">
            {currentTemplate.abilities.map(ability => (
              <button
                ...
                style={{ 
                  backgroundColor: selectedAbility === ability.id ? '#e94560' : '#1a1a2e',
                  border: '2px solid',
                  borderColor: selectedAbility === ability.id ? '#e94560' : '#2a2a4a',
                  color: '#ffffff'
                }}
      ...
      {selectedAbility && selectedTarget && (
        <div className="rounded-2xl p-5 mb-5 text-center" style={{ backgroundColor: '#16213e', border: '2px solid #2a2a4a' }}>
          ...
          <button 
            ...
            style={{ backgroundColor: 'transparent', border: '2px solid #2a2a4a', color: '#6b6b80' }}
      ...
      <div className="rounded-2xl p-4 max-h-[200px] overflow-y-auto" style={{ backgroundColor: '#16213e', border: '2px solid #2a2a4a' }}>
        <div className="text-xs font-semibold uppercase tracking-wider mb-3" style={{ color: '#6b6b80' }}>Game Log</div>
        {actions.length === 0 ? (
          <div className="text-sm py-1.5" style={{ color: '#a0a0b8', borderBottom: '1px solid #2a2a4a' }}>Game started. Waiting for actions...</div>
        ) : (
          actions.map((action, i) => (
            <div key={i} className="text-sm py-1.5" style={{ color: '#a0a0b8', borderBottom: '1px solid #2a2a4a' }}>
              <span style={{ color: '#e94560', fontWeight: 600 }}>{action?.actorName}</span> {action?.verb}{' '}
              <span style={{ color: '#4ade80', fontWeight: 600 }}>{action?.targetNames}</span>
            </div>
          ))
        )}
      </div>
